...
class CEmptyImage : public CAbstractImage
{
public:

	CEmptyImage(int iWidth, int iHeight, int iScaleMode, const RGBQUAD *prgbBackground);
	virtual ~CEmptyImage();

private:

	CEmptyImage(const CEmptyImage &rhs); // disallow
	CEmptyImage &operator=(const CEmptyImage &rhs); // disallow
