...
int main() {
    FILE* out = nullptr;
    fopen_s(&out, "d:\\vfs\\test_sector_api3.txt", "wb");
    if (!out) { printf("Cannot open output file\n"); return 1; }

    fprintf(out, "=== Test 1: \u884c\u4e1a\u677f\u5757\u6210\u5206\u80a1 (node=hangye_ZA01 \u519c\u4e1a) ===\n");
    {
        std::wstring path = L"/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page=1&num=5&sort=changepercent&asc=0&node=hangye_ZA01&_s_r_a=auto";
    ...
    fprintf(out, "\n=== Test 2: \u5730\u57df\u677f\u5757\u6210\u5206\u80a1 (node=diyu_440000 \u5e7f\u4e1c) ===\n");
    {
        std::wstring path = L"/quotes_service/api/json_v2.php/Market_Center.getHQNodeData?page=1&num=5&sort=changepercent&asc=0&node=diyu_440000&_s_r_a=auto";
        std::string response = SendHttpGet(L"vip.stock.finance.sina.com.cn", path, 80, false);
        fprintf(out, "Response length: %zu\n", response.length());
        fprintf(out, "First 2000: %.2000s\n", response.c_str());
    }

    fprintf(out, "\n=== Test 3: \u4e1c\u65b9\u8d22\u5bcc\u884c\u4e1a\u677f\u5757 (HTTPS) ===\n");
    {
        std::wstring path = L"/api/qt/clist/get?pn=1&pz=10&po=1&np=1&fltt=2&invt=2&fid=f3&fs=m:90+t:2&fields=f2,f3,f4,f12,f14";
        std::string response = SendHttpGet(L"push2.eastmoney.com", path, 443, true);
        fprintf(out, "Response length: %zu\n", response.length());
        fprintf(out, "First 2000: %.2000s\n", response.c_str());
    }

    fprintf(out, "\n=== Test 4: \u4e1c\u65b9\u8d22\u5bcc\u6307\u6570\u5217\u8868 (HTTPS) ===\n");
    {
    ...
    fprintf(out, "\n=== Test 5: \u4e1c\u65b9\u8d22\u5bcc\u6982\u5ff5\u677f\u5757 (HTTPS) ===\n");
    {
        std::wstring path = L"/api/qt/clist/get?pn=1&pz=10&po=1&np=1&fltt=2&invt=2&fid=f3&fs=m:90+t:3&fields=f2,f3,f4,f12,f14";
        std::string response = SendHttpGet(L"push2.eastmoney.com", path, 443, true);
        fprintf(out, "Response length: %zu\n", response.length());
        fprintf(out, "First 2000: %.2000s\n", response.c_str());
    }
    ...
    fclose(out);
    printf("Done. Output: d:\\vfs\\test_sector_api3.txt\n");
    return 0;
}
