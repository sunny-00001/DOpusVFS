...
set SOURCES=src/IPFSVFS.cpp IPFSClient.cpp
...
if defined RESFILE (
    cl.exe %CXXFLAGS% %INCLUDES% %DEFINES% %SOURCES% /Fe"%OUTFILE%" /link /DEF:IPFSVFS.def /NOIMPLIB %LIBS% %RESFILE%
) else (
    cl.exe %CXXFLAGS% %INCLUDES% %DEFINES% %SOURCES% /Fe"%OUTFILE%" /link /DEF:IPFSVFS.def /NOIMPLIB %LIBS%
)
